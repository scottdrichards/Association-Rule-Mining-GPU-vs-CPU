...
/*******************  CUDA DEVICE TO HOST COPY  ***************************/
    ...
    fprintf(fout,"{item} => {itemset}:  support %%       confidence %% \n");
    fprintf(fout,"--------------------------------------------\n\n");

    for (i = 0; i < num_of_transactions*EC_size*num_of_itemsets; i++)  {
        if (strcmp(op_element[i][0],"") == 0)  continue;
        fprintf(fout,"{%s} => {%s}:  ",op_element[i][0],op_element[i][1]);

        for (j = 0; j < 2; j++)  {
            fprintf(fout,"%.4f %%        ",op_sc[i][j]);
        }
        fprintf(fout,"\n");
    }
/*******************  END  *****************************/
...
/*******************  CUDA KERNEL LAUNCH  *********************************/
    for (n = 2; n < num_of_itemsets; n++)  {
        cudaMemset3D(op_element_gpuPitchPtr,0,op_element_gpuVolSizeBytes);
        cudaMemset3D(op_sc_gpuPitchPtr,0.0000,op_sc_gpuVolSizeBytes);

	if (n % 2 == 0)  {
	    cudaMemset3D(EC_set2_gpuPitchPtr,0,EC_set2_gpuVolSizeBytes);
    	    cudaEC_higherlen <<< 1, 27 >>> (db_gpuDim, db_tran_gpuPitchPtr, EC_set1_gpuPitchPtr, EC_set2_gpuPitchPtr, op_element_gpuPitchPtr, op_sc_gpuPitchPtr, EC_size, n);
	}
	else  {
	    cudaMemset3D(EC_set1_gpuPitchPtr,0,EC_set1_gpuVolSizeBytes);
	    cudaEC_higherlen <<< 1, 27 >>> (db_gpuDim, db_tran_gpuPitchPtr, EC_set2_gpuPitchPtr, EC_set1_gpuPitchPtr, op_element_gpuPitchPtr, op_sc_gpuPitchPtr, EC_size, n);
	}
...
/*******************  CUDA DEVICE TO HOST COPY  ***************************/
    ...
    for (i = 0; i < num_of_transactions*EC_size*num_of_itemsets; i++)  {
        if (strcmp(op_element[i][0],"") == 0)  continue;
        fprintf(fout,"{%s} => {",op_element[i][0]);

	for (j = 1; j < n; j++)  {
            fprintf(fout,"%s,",op_element[i][j]);
        }
	fprintf(fout,"%s}:  ",op_element[i][n]);
...
/*******************  END  *****************************/
    printf("++++++++++  End  ++++++++++\n");
    printf("\n result.txt file has been written successfully\n\n");

    cudaFree(db_tran_gpuPitchPtr.ptr);
    cudaFree(EC_len2_gpuPitchPtr.ptr);
    cudaFree(EC_set1_gpuPitchPtr.ptr);
    cudaFree(EC_set2_gpuPitchPtr.ptr);
