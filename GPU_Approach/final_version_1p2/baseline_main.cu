...
/*******************  OTHER INITIALIZATIONS  ********************************/
    ...
    float op_sc[num_of_transactions*EC_size*2][2];

    char *op_element[num_of_transactions*EC_size*2][num_of_itemsets];  // say (A,B) then o/p is A => B and B => A, so *2 num of rows
...
/*******************  CUDA HOST TO DEVICE COPY  ***************************************/
    ...
    ********************  END  *********************/
    ...
    dim3 op_element_gpuDim(100,num_of_transactions*EC_size*2,num_of_itemsets);
    ...
    dim3 op_sc_gpuDim(1,num_of_transactions*EC_size*2,8);
...
/*************  Redundant (Please Ignore) *****************
    ...
    for (k = 0; k < 27; k++)  {
        for (i = 0; i < num_of_transactions*EC_size; i++)  {
            ...
            printf("{%s,%s}: ",EC_set1[i][(k*(num_of_transactions+num_of_itemsets))+0],EC_set1[i][(k*(num_of_transactions+num_of_itemsets))+1]);
...
/*******************  CUDA DEVICE TO HOST COPY  ***************************/
    ...
    size_t op_slicePitch = op_pitch * num_of_transactions * EC_size * 2;
    char *op_slice;
    ...
    size_t op_sc_slicePitch = op_sc_pitch * num_of_transactions * EC_size * 2;
    float *op_sc_slice;
    float *op_sc_element;
